...
def main():
    ...
    parser.add_argument('--model_path', type=str, default='../experiments/pretrained_models/DFDAN_x4.pth')
    parser.add_argument('--input', type=str, default='../datasets/set14/mod4/LRx4', help='input test image folder')
    parser.add_argument('--output', type=str, default='results/DFDAN', help='output folder')
    args = parser.parse_args()

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    # set up model
    model = DFDAN(in_channels=3, channels=56, num_block=8, out_channels=3, upscale=4)
